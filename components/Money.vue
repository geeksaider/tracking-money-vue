...
function static_update() {
    historyData.value = [];
    let staticArr = JSON.parse(getItem("dateProject"));
    let dateStatic = new Date();
    if (typeStatic.value == "week") {
        for (let i = 0; i < staticArr.length; i++) {
            staticArr[i].date = new Date(Date.parse(staticArr[i].date));
            if (
                staticArr[i].date.getDate() >= dateStatic.getDate() - 7 &&
                staticArr[i].date.getMonth() == dateStatic.getMonth() &&
                staticArr[i].date.getFullYear() == dateStatic.getFullYear()
            ) {
                console.log(staticArr[i]);
                historyData.value.push(staticArr[i]);
            }
        }
    } else if (typeStatic.value == "month") {
        for (let i = 0; i < staticArr.length; i++) {
            staticArr[i].date = new Date(Date.parse(staticArr[i].date));
            if (
                staticArr[i].date.getMonth() == dateStatic.getMonth() &&
    ...
    } else if (typeStatic.value == "year") {
        dateStatic = dateStatic.getFullYear();
        for (let i = 0; i < staticArr.length; i++) {
            staticArr[i].date = new Date(Date.parse(staticArr[i].date));
            if (staticArr[i].date.getFullYear() == dateStatic) {
                console.log(staticArr[i]);
                historyData.value.push(staticArr[i]);
            }
        }
...
function deleteCategory(category) {
    ...
    console.log(categorysArr.value);
}
...
<template>
    <div
        class="bg-white px-40 py-20 flex gap-20 min-w-max justify-center rounded-2xl my-20"
    >
        <div class="flex flex-col gap-3">
            <h1 class="text-4xl font-bold">Статистика</h1>
            <div class="mt-3">
                <input
                    type="radio"
                    name="static"
                    ...
                    v-model="typeStatic"
                />
                <label for="static_week">Статистикак за неделю</label>
            </div>
            <div>
                <input
                    type="radio"
                    ...
                    v-model="typeStatic"
                />
                <label for="static_month">Статистикак за месяц</label>
            </div>
            <div>
                <input
                    type="radio"
                    ...
                    v-model="typeStatic"
                />
                <label for="static_year">Статистикак за год</label>
            </div>
            <button
                class="bg-stone-700 rounded-xl px-4 py-1 text-white hover:scale-110 transition-all"
                @click="static_update"
            >
                Узнать
            </button>
                    ...
                    >
                        ...
                        >
                            ...
                            <button
                                ...
                                @click="
                                    () => deleteArr(object.date, object.cost)
                                "
                            >
                                Удалить
                            </button>
